...
Verification status as of this investigation (paper-exact hyperparameters + `augment: false`, single seed):
- **mnist_m**: closely reproduces the paper on both source and target — SIDDA `final_model`/`best_model_DA_loss` checkpoints hit 94.4%/77.6% source/target vs. the paper's 95.31%/76.24%, and no longer shows the catastrophic warmup→DA collapse seen with augmentation on (61.5%/36.1%, worse than not doing DA at all).
- **shapes**: source-domain collapse is fully fixed (SIDDA stays ≥99% throughout, vs. crashing to 34% mid-training with augmentation on) — but the actual **DA target-domain benefit is not reliably reproduced**: `final_model` target accuracy (36.5%) is *worse* than the plain CE baseline (65.2%), despite the paper reporting CNN-DA at 78.20% vs. CNN at 50.47%. This is a known, open, unresolved gap — not yet root-caused.
- **astro_objects, gz_evo, mrssc2**: not yet re-verified under `augment: false` / paper-exact hyperparameters.
